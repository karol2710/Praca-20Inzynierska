...
const isValidNamespace = (name: string) => /^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/.test(name);

// CPU format validation
const isValidCPU = (cpu: string) => /^(\d+m|\d+(\.\d+)?)$/.test(cpu);

// Memory format validation
const isValidMemory = (mem: string) => /^(\d+)([KMG]i|[KMG]|[KMGTPEZY]i?)$/.test(mem);

// Storage format validation
const isValidStorage = (storage: string) => /^(\d+)([KMG]i|[KMG]|[KMGTPEZY]i?)$/.test(storage);
```
