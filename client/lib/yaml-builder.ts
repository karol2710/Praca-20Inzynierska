...
function buildContainerSpec(container: Container): Record<string, any> {
  ...
  if (container.lifecycle) {
    ...
    if (container.lifecycle.postStart)
      lifecycle.postStart = container.lifecycle.postStart;
    if (container.lifecycle.preStop)
      lifecycle.preStop = container.lifecycle.preStop;
    if (Object.keys(lifecycle).length > 0) spec.lifecycle = lifecycle;
  }

  if (container.terminationMessagePath)
    spec.terminationMessagePath = container.terminationMessagePath;
  if (container.terminationMessagePolicy)
    spec.terminationMessagePolicy = container.terminationMessagePolicy;

  if (container.env && container.env.length > 0) spec.env = container.env;
  if (container.envFrom && container.envFrom.length > 0)
    spec.envFrom = container.envFrom;
  if (container.ports && container.ports.length > 0)
    spec.ports = container.ports;
  ...
  if (container.volumeMounts && container.volumeMounts.length > 0)
    spec.volumeMounts = container.volumeMounts;
  if (container.volumeDevices && container.volumeDevices.length > 0)
    spec.volumeDevices = container.volumeDevices;
  ...
  if (container.resizePolicy && container.resizePolicy.length > 0)
    spec.resizePolicy = container.resizePolicy;
  if (container.restartPolicy) spec.restartPolicy = container.restartPolicy;
  if (container.restartPolicyRules && container.restartPolicyRules.length > 0)
    spec.restartPolicyRules = container.restartPolicyRules;
...
function buildPodSpec(
  ...
  spec.containers = containers.map((container) =>
    buildContainerSpec(container),
  );

  if (config.initContainers && config.initContainers.length > 0) {
    spec.initContainers = config.initContainers.map((container: Container) =>
      buildContainerSpec(container),
    );
  }

  if (config.ephemeralContainers && config.ephemeralContainers.length > 0) {
    spec.ephemeralContainers = config.ephemeralContainers.map(
      (container: Container) => buildContainerSpec(container),
    );
  }

  if (config.podDeathTime) spec.activeDeadlineSeconds = config.podDeathTime;
  if (config.terminationGracePeriodSeconds)
  ...
  if (config.automountServiceAccountToken !== undefined)
  ...
  if (config.enableServiceLinks !== undefined)
  ...
  if (config.shareProcessNamespace !== undefined)
    spec.shareProcessNamespace = config.shareProcessNamespace;

  if (config.dnsConfig) {
    const dnsConfig: Record<string, any> = {};
    if (config.dnsConfig.nameServers && config.dnsConfig.nameServers.length > 0)
      dnsConfig.nameservers = config.dnsConfig.nameServers;
    if (config.dnsConfig.searches && config.dnsConfig.searches.length > 0)
      dnsConfig.searches = config.dnsConfig.searches;
    if (config.dnsConfig.options && config.dnsConfig.options.length > 0)
      dnsConfig.options = config.dnsConfig.options;
    if (Object.keys(dnsConfig).length > 0) spec.dnsConfig = dnsConfig;
  }

  if (config.hostAliases && config.hostAliases.length > 0)
    spec.hostAliases = config.hostAliases;

  if (config.tolerations && config.tolerations.length > 0)
    spec.tolerations = config.tolerations;

  if (
    config.topologySpreadConstraints &&
    config.topologySpreadConstraints.length > 0
  )
  ...
  if (config.affinity) {
    ...
    if (config.affinity.nodeAffinity) {
      ...
      const requiredTerms =
        config.affinity.nodeAffinity.requiredDuringScheduling?.nodeAffinityTerm;
      const hasRequiredContent =
        requiredTerms &&
        ((requiredTerms.matchExpressions &&
          requiredTerms.matchExpressions.length > 0) ||
      ...
      const preferredTerms =
        config.affinity.nodeAffinity.preferredDuringScheduling
          ?.nodeAffinityTerm;
      const hasPreferredContent =
        preferredTerms &&
        ((preferredTerms.matchExpressions &&
          preferredTerms.matchExpressions.length > 0) ||
          (preferredTerms.matchFields &&
            preferredTerms.matchFields.length > 0));
      if (hasPreferredContent) {
        const weight =
          config.affinity.nodeAffinity.preferredDuringScheduling?.weight || 1;
    ...
    if (config.affinity.podAffinity) {
      ...
      const requiredTerm =
        config.affinity.podAffinity.requiredDuringScheduling?.podAffinityTerm;
      ...
      const preferredTerm =
        config.affinity.podAffinity.preferredDuringScheduling?.podAffinityTerm;
      if (preferredTerm?.topologyKey) {
        podAffinity.preferredDuringSchedulingIgnoredDuringExecution = [
          {
            weight:
              config.affinity.podAffinity.preferredDuringScheduling?.weight ||
              1,
    ...
    if (config.affinity.podAntiAffinity) {
      ...
      const requiredTerm =
        config.affinity.podAntiAffinity.requiredDuringScheduling
          ?.podAffinityTerm;
      if (requiredTerm?.topologyKey) {
      ...
      const preferredTerm =
        config.affinity.podAntiAffinity.preferredDuringScheduling
          ?.podAffinityTerm;
      if (preferredTerm?.topologyKey) {
  ...
  if (
    ...
    typeof config.imagePullSecrets === "object" &&
    Object.keys(config.imagePullSecrets).length > 0
  ) {
...
export function generatePodYAML(
  podName: string,
  ...
  if (podConfig.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds = podConfig.deletionGracePeriodSeconds;
...
function buildWorkloadYAML(
  ...
  if (namespace) metadata.namespace = namespace;
  if (config.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds = config.deletionGracePeriodSeconds;
  ...
  if (config.spec?.minReadySeconds !== undefined)
    spec.minReadySeconds = config.spec.minReadySeconds;
  if (config.spec?.progressDeadlineSeconds !== undefined)
    spec.progressDeadlineSeconds = config.spec.progressDeadlineSeconds;
  if (config.spec?.revisionHistoryLimit !== undefined)
    spec.revisionHistoryLimit = config.spec.revisionHistoryLimit;
    ...
    if (config.spec.strategy.rollingUpdate) {
      ...
      if (config.spec.strategy.rollingUpdate.maxUnavailable !== undefined) {
        rollingUpdate.maxUnavailable =
          config.spec.strategy.rollingUpdate.maxUnavailable;
      }
  ...
  if (config.spec?.updateStrategy) {
    ...
    if (config.spec.updateStrategy.type)
      updateStrategy.type = config.spec.updateStrategy.type;
    if (config.spec.updateStrategy.rollingUpdate) {
      const rollingUpdate: Record<string, any> = {};
      if (config.spec.updateStrategy.rollingUpdate.maxSurge !== undefined) {
        rollingUpdate.maxSurge =
          config.spec.updateStrategy.rollingUpdate.maxSurge;
      }
      if (
        config.spec.updateStrategy.rollingUpdate.maxUnavailable !== undefined
      ) {
        rollingUpdate.maxUnavailable =
          config.spec.updateStrategy.rollingUpdate.maxUnavailable;
      }
      if (config.spec.updateStrategy.rollingUpdate.partition !== undefined) {
        rollingUpdate.partition =
          config.spec.updateStrategy.rollingUpdate.partition;
      }
      if (Object.keys(rollingUpdate).length > 0) {
  ...
  if (config.spec?.podManagementPolicy)
    spec.podManagementPolicy = config.spec.podManagementPolicy;
  ...
  if (config.template) {
    ...
    const templateLabels = config.template.labels
    ...
    if (
      ...
      Object.keys(config.template.annotations).length > 0
    ) {
...
function addTrailingNewlinesToMultilineValues(
...
export function generateReplicaSetYAML(
...
export function generateStatefulSetYAML(
  statefulSetName: string,
  statefulSetConfig: Record<string, any>,
  containers: Container[],
  namespace?: string,
): string {
  const metadata: Record<string, any> = {
  ...
  if (namespace) metadata.namespace = namespace;
  if (statefulSetConfig.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds =
      statefulSetConfig.deletionGracePeriodSeconds;

  if (
    statefulSetConfig.annotations &&
    Object.keys(statefulSetConfig.annotations).length > 0
  ) {
    metadata.annotations = statefulSetConfig.annotations;
  }

  if (
    statefulSetConfig.labels &&
    Object.keys(statefulSetConfig.labels).length > 0
  ) {
    metadata.labels = statefulSetConfig.labels;
  }

  if (
    statefulSetConfig.ownerReferences &&
    statefulSetConfig.ownerReferences.length > 0
  ) {
  ...
  if (statefulSetConfig.spec?.replicas !== undefined)
    spec.replicas = statefulSetConfig.spec.replicas;
  if (statefulSetConfig.spec?.minReadySeconds !== undefined)
    spec.minReadySeconds = statefulSetConfig.spec.minReadySeconds;
  if (statefulSetConfig.spec?.revisionHistoryLimit !== undefined)
    spec.revisionHistoryLimit = statefulSetConfig.spec.revisionHistoryLimit;

  if (statefulSetConfig.spec?.ordinals) {
    const ordinals: Record<string, any> = {};
    if (statefulSetConfig.spec.ordinals.start !== undefined)
      ordinals.start = statefulSetConfig.spec.ordinals.start;
  ...
  if (statefulSetConfig.spec?.persistentVolumeClaimRetentionPolicy) {
    ...
    if (
      statefulSetConfig.spec.persistentVolumeClaimRetentionPolicy.whenDeleted
    ) {
      pvcRetention.whenDeleted =
        statefulSetConfig.spec.persistentVolumeClaimRetentionPolicy.whenDeleted;
    }
    if (
      statefulSetConfig.spec.persistentVolumeClaimRetentionPolicy.whenScaled
    ) {
      pvcRetention.whenScaled =
        statefulSetConfig.spec.persistentVolumeClaimRetentionPolicy.whenScaled;
    }
  ...
  if (statefulSetConfig.spec?.serviceName)
    spec.serviceName = statefulSetConfig.spec.serviceName;
  if (statefulSetConfig.spec?.podManagementPolicy)
    spec.podManagementPolicy = statefulSetConfig.spec.podManagementPolicy;

  if (statefulSetConfig.spec?.updateStrategy) {
    const updateStrategy: Record<string, any> = {};
    if (statefulSetConfig.spec.updateStrategy.type)
      updateStrategy.type = statefulSetConfig.spec.updateStrategy.type;
    if (statefulSetConfig.spec.updateStrategy.rollingUpdate) {
      const rollingUpdate: Record<string, any> = {};
      if (
        statefulSetConfig.spec.updateStrategy.rollingUpdate.maxUnavailable !==
        undefined
      ) {
        rollingUpdate.maxUnavailable =
          statefulSetConfig.spec.updateStrategy.rollingUpdate.maxUnavailable;
      ...
      if (
        statefulSetConfig.spec.updateStrategy.rollingUpdate.partition !==
        undefined
      ) {
        rollingUpdate.partition =
          statefulSetConfig.spec.updateStrategy.rollingUpdate.partition;
  ...
  if (statefulSetConfig.template) {
    spec.template = {
      metadata: {},
      spec: cleanEmptyValues(
        buildPodSpec(statefulSetConfig.template, containers),
      ),
    };

    // Add app label to pod template
    const templateLabels = statefulSetConfig.template.labels
      ? { ...statefulSetConfig.template.labels }
      : {};
    ...
    if (
      statefulSetConfig.template.annotations &&
      Object.keys(statefulSetConfig.template.annotations).length > 0
    ) {
  ...
  if (
    statefulSetConfig.spec?.volumeClaimTemplates &&
    statefulSetConfig.spec.volumeClaimTemplates.length > 0
  ) {
    spec.volumeClaimTemplates = statefulSetConfig.spec.volumeClaimTemplates.map(
      (template: Record<string, any>) => {
        ...
        if (template.metadata?.deletionGracePeriodSeconds) {
          vctMetadata.deletionGracePeriodSeconds =
            template.metadata.deletionGracePeriodSeconds;
        }
        if (
          template.metadata?.annotations &&
          Object.keys(template.metadata.annotations).length > 0
        ) {
          vctMetadata.annotations = template.metadata.annotations;
        }
        if (
          template.metadata?.labels &&
          Object.keys(template.metadata.labels).length > 0
        ) {
        ...
        if (
          template.spec?.accessModes &&
          template.spec.accessModes.length > 0
        ) {
          vctSpec.accessModes = template.spec.accessModes;
        }
        if (template.spec?.storageClassName)
          vctSpec.storageClassName = template.spec.storageClassName;
        if (template.spec?.volumeName)
          vctSpec.volumeName = template.spec.volumeName;
        if (template.spec?.volumeMode)
          vctSpec.volumeMode = template.spec.volumeMode;
        if (template.spec?.volumeAttributesClassName)
          vctSpec.volumeAttributesClassName =
            template.spec.volumeAttributesClassName;

        if (template.spec?.resources) {
          const resources: Record<string, any> = {};
          if (
            template.spec.resources.limits &&
            Object.keys(template.spec.resources.limits).length > 0
          ) {
            resources.limits = template.spec.resources.limits;
          }
          if (
            template.spec.resources.requests &&
            Object.keys(template.spec.resources.requests).length > 0
          ) {
        ...
        if (template.spec?.dataSource) {
          ...
          if (template.spec.dataSource.apiGroup)
            dataSource.apiGroup = template.spec.dataSource.apiGroup;
          if (template.spec.dataSource.kind)
            dataSource.kind = template.spec.dataSource.kind;
          if (template.spec.dataSource.name)
            dataSource.name = template.spec.dataSource.name;
          if (Object.keys(dataSource).length > 0)
            vctSpec.dataSource = dataSource;
        }

        if (template.spec?.dataSourceRef) {
          const dataSourceRef: Record<string, any> = {};
          if (template.spec.dataSourceRef.apiGroup)
            dataSourceRef.apiGroup = template.spec.dataSourceRef.apiGroup;
          if (template.spec.dataSourceRef.kind)
            dataSourceRef.kind = template.spec.dataSourceRef.kind;
          if (template.spec.dataSourceRef.name)
            dataSourceRef.name = template.spec.dataSourceRef.name;
          if (template.spec.dataSourceRef.namespace)
            dataSourceRef.namespace = template.spec.dataSourceRef.namespace;
          if (Object.keys(dataSourceRef).length > 0)
            vctSpec.dataSourceRef = dataSourceRef;
        }
...
export function generateDaemonSetYAML(
  daemonSetName: string,
  daemonSetConfig: Record<string, any>,
  containers: Container[],
  namespace?: string,
): string {
  return buildWorkloadYAML(
...
export function generateJobYAML(
  jobName: string,
  ...
  if (namespace) metadata.namespace = namespace;
  if (jobConfig.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds = jobConfig.deletionGracePeriodSeconds;
  ...
  if (jobConfig.spec?.activeDeadlineSeconds !== undefined)
    spec.activeDeadlineSeconds = jobConfig.spec.activeDeadlineSeconds;
  if (jobConfig.spec?.backoffLimit !== undefined)
    spec.backoffLimit = jobConfig.spec.backoffLimit;
  if (jobConfig.spec?.backoffLimitPerIndex !== undefined)
    spec.backoffLimitPerIndex = jobConfig.spec.backoffLimitPerIndex;
  if (jobConfig.spec?.completionMode)
    spec.completionMode = jobConfig.spec.completionMode;
  if (jobConfig.spec?.completions !== undefined)
    spec.completions = jobConfig.spec.completions;
  if (jobConfig.spec?.manualSelector !== undefined)
    spec.manualSelector = jobConfig.spec.manualSelector;
  if (jobConfig.spec?.maxFailedIndexes !== undefined)
    spec.maxFailedIndexes = jobConfig.spec.maxFailedIndexes;
  if (jobConfig.spec?.parallelism !== undefined)
    spec.parallelism = jobConfig.spec.parallelism;
  if (jobConfig.spec?.podReplacementPolicy)
    spec.podReplacementPolicy = jobConfig.spec.podReplacementPolicy;
  if (jobConfig.spec?.ttlSecondsAfterFinished !== undefined)
    spec.ttlSecondsAfterFinished = jobConfig.spec.ttlSecondsAfterFinished;
  ...
  if (
    jobConfig.spec?.podFailurePolicy?.rules &&
    jobConfig.spec.podFailurePolicy.rules.length > 0
  ) {
    spec.podFailurePolicy = {
      rules: jobConfig.spec.podFailurePolicy.rules.map(
        (rule: Record<string, any>) => {
          ...
          if (rule.onPodConditions)
            cleanedRule.onPodConditions = rule.onPodConditions;
  ...
  if (
    jobConfig.spec?.successPolicy?.rules &&
    jobConfig.spec.successPolicy.rules.length > 0
  ) {
    spec.successPolicy = {
      rules: jobConfig.spec.successPolicy.rules.map(
        (rule: Record<string, any>) => {
          const cleanedRule: Record<string, any> = {};
          if (rule.succeededCount !== undefined)
            cleanedRule.succeededCount = rule.succeededCount;
          if (rule.succeededIndexes)
            cleanedRule.succeededIndexes = rule.succeededIndexes;
          return cleanEmptyValues(cleanedRule);
        },
      ),
    };
  ...
  if (jobConfig.template) {
    ...
    // Add app label to pod template
    const templateLabels = jobConfig.template.labels
      ? { ...jobConfig.template.labels }
      : {};
    ...
    if (
      jobConfig.template.annotations &&
      Object.keys(jobConfig.template.annotations).length > 0
    ) {
...
export function generateCronJobYAML(
  cronJobName: string,
  cronJobConfig: Record<string, any>,
  containers: Container[],
  namespace?: string,
): string {
  const metadata: Record<string, any> = {
  ...
  if (namespace) metadata.namespace = namespace;
  if (cronJobConfig.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds =
      cronJobConfig.deletionGracePeriodSeconds;

  if (
    cronJobConfig.annotations &&
    Object.keys(cronJobConfig.annotations).length > 0
  ) {
  ...
  if (
    cronJobConfig.ownerReferences &&
    cronJobConfig.ownerReferences.length > 0
  ) {
  ...
  if (cronJobConfig.spec?.concurrencyPolicy)
    spec.concurrencyPolicy = cronJobConfig.spec.concurrencyPolicy;
  if (cronJobConfig.spec?.failedJobsHistoryLimit !== undefined)
    spec.failedJobsHistoryLimit = cronJobConfig.spec.failedJobsHistoryLimit;
  if (cronJobConfig.spec?.startingDeadlineSeconds !== undefined)
    spec.startingDeadlineSeconds = cronJobConfig.spec.startingDeadlineSeconds;
  if (cronJobConfig.spec?.successfulJobsHistoryLimit !== undefined)
    spec.successfulJobsHistoryLimit =
      cronJobConfig.spec.successfulJobsHistoryLimit;
  ...
  if (cronJobConfig.spec?.jobTemplate) {
    ...
    if (
      cronJobConfig.spec.jobTemplate.metadata?.labels &&
      Object.keys(cronJobConfig.spec.jobTemplate.metadata.labels).length > 0
    ) {
    ...
    if (
      cronJobConfig.spec.jobTemplate.metadata?.annotations &&
      Object.keys(cronJobConfig.spec.jobTemplate.metadata.annotations).length >
        0
    ) {
    ...
    if (cronJobConfig.spec.jobTemplate.spec) {
      ...
      if (jobSpec.activeDeadlineSeconds !== undefined)
        jobSpecObj.activeDeadlineSeconds = jobSpec.activeDeadlineSeconds;
      if (jobSpec.backoffLimit !== undefined)
        jobSpecObj.backoffLimit = jobSpec.backoffLimit;
      if (jobSpec.backoffLimitPerIndex !== undefined)
        jobSpecObj.backoffLimitPerIndex = jobSpec.backoffLimitPerIndex;
      if (jobSpec.completionMode)
        jobSpecObj.completionMode = jobSpec.completionMode;
      if (jobSpec.completions !== undefined)
        jobSpecObj.completions = jobSpec.completions;
      if (jobSpec.manualSelector !== undefined)
        jobSpecObj.manualSelector = jobSpec.manualSelector;
      if (jobSpec.maxFailedIndexes !== undefined)
        jobSpecObj.maxFailedIndexes = jobSpec.maxFailedIndexes;
      if (jobSpec.parallelism !== undefined)
        jobSpecObj.parallelism = jobSpec.parallelism;
      if (jobSpec.podReplacementPolicy)
        jobSpecObj.podReplacementPolicy = jobSpec.podReplacementPolicy;
      if (jobSpec.ttlSecondsAfterFinished !== undefined)
        jobSpecObj.ttlSecondsAfterFinished = jobSpec.ttlSecondsAfterFinished;
      ...
      if (
        jobSpec.podFailurePolicy?.rules &&
        jobSpec.podFailurePolicy.rules.length > 0
      ) {
        jobSpecObj.podFailurePolicy = {
          rules: jobSpec.podFailurePolicy.rules.map(
            (rule: Record<string, any>) => {
              const cleanedRule: Record<string, any> = {};
              if (rule.action) cleanedRule.action = rule.action;
              if (rule.onExitCodes) cleanedRule.onExitCodes = rule.onExitCodes;
      ...
      if (
        jobSpec.successPolicy?.rules &&
        jobSpec.successPolicy.rules.length > 0
      ) {
        jobSpecObj.successPolicy = {
          rules: jobSpec.successPolicy.rules.map(
            (rule: Record<string, any>) => {
              const cleanedRule: Record<string, any> = {};
              if (rule.succeededCount !== undefined)
                cleanedRule.succeededCount = rule.succeededCount;
    ...
    if (cronJobConfig.spec.jobTemplate.template) {
      spec.jobTemplate.spec.template = {
        metadata: {},
        spec: cleanEmptyValues(
          buildPodSpec(cronJobConfig.spec.jobTemplate.template, containers),
        ),
      };

      // Add app label to pod template
      const templateLabels = cronJobConfig.spec.jobTemplate.template.labels
        ? { ...cronJobConfig.spec.jobTemplate.template.labels }
        : {};
      ...
      if (
        cronJobConfig.spec.jobTemplate.template.annotations &&
        Object.keys(cronJobConfig.spec.jobTemplate.template.annotations)
          .length > 0
      ) {
      ...
      spec.jobTemplate.spec.template.metadata = cleanEmptyValues(
...
export function generateResourceYAML(
  resourceName: string,
  resourceType: string,
  ...
  if (namespace) metadata.namespace = namespace;
  if (resourceConfig.deletionGracePeriodSeconds)
    metadata.deletionGracePeriodSeconds =
      resourceConfig.deletionGracePeriodSeconds;

  if (
    resourceConfig.annotations &&
    Object.keys(resourceConfig.annotations).length > 0
  ) {
  ...
  if (
    resourceConfig.ownerReferences &&
    resourceConfig.ownerReferences.length > 0
  ) {
  ...
  if (resourceType === "Service" && resourceConfig.spec) {
    ...
    if (serviceSpec.publishNotReadyAddresses !== undefined)
      spec.publishNotReadyAddresses = serviceSpec.publishNotReadyAddresses;
    if (serviceSpec.trafficDistribution)
      spec.trafficDistribution = serviceSpec.trafficDistribution;
    if (serviceSpec.sessionAffinity)
      spec.sessionAffinity = serviceSpec.sessionAffinity;

    // Handle sessionAffinityTimeout with nested structure
    if (
      serviceSpec.sessionAffinityTimeout !== undefined &&
      serviceSpec.sessionAffinity === "ClientIP"
    ) {
        ...
        if (rule.filters && rule.filters.length > 0) {
          ruleObj.filters = rule.filters.map((filter: Record<string, any>) => {
            ...
            if (filter.requestHeaderModifier)
              filterObj.requestHeaderModifier = filter.requestHeaderModifier;
            if (filter.responseHeaderModifier)
              filterObj.responseHeaderModifier = filter.responseHeaderModifier;
            if (filter.requestRedirect)
              filterObj.requestRedirect = filter.requestRedirect;
            ...
            if (filter.requestMirror) {
              ...
              if (filter.requestMirror.backendRef) {
                ...
                if (filter.requestMirror.backendRef.name)
                  filterObj.requestMirror.backendRef.name =
                    filter.requestMirror.backendRef.name;
                if (filter.requestMirror.backendRef.port !== undefined)
                  filterObj.requestMirror.backendRef.port =
                    filter.requestMirror.backendRef.port;
                filterObj.requestMirror.backendRef.namespace =
                  namespace || "default";
              }
              if (filter.requestMirror.percent !== undefined)
                filterObj.requestMirror.percent = filter.requestMirror.percent;
              if (filter.requestMirror.fraction)
                filterObj.requestMirror.fraction =
          ...
          ruleObj.backendRefs = rule.backendRefs.map(
            (backend: Record<string, any>) => {
              ...
              if (backend.filters && backend.filters.length > 0) {
                backendObj.filters = backend.filters.map(
                  (bFilter: Record<string, any>) => {
                    const bFilterObj: Record<string, any> = {
                      type: bFilter.type,
                    };
                    if (bFilter.requestHeaderModifier)
                      bFilterObj.requestHeaderModifier =
                        bFilter.requestHeaderModifier;
                    if (bFilter.responseHeaderModifier)
                      bFilterObj.responseHeaderModifier =
                        bFilter.responseHeaderModifier;
                    if (bFilter.requestRedirect)
                      bFilterObj.requestRedirect = bFilter.requestRedirect;
                    if (bFilter.urlRewrite)
                      bFilterObj.urlRewrite = bFilter.urlRewrite;

                    if (bFilter.requestMirror) {
                      bFilterObj.requestMirror = {};
                      if (bFilter.requestMirror.backendRef) {
                        bFilterObj.requestMirror.backendRef = {};
                        if (bFilter.requestMirror.backendRef.name)
                          bFilterObj.requestMirror.backendRef.name =
                            bFilter.requestMirror.backendRef.name;
                        if (bFilter.requestMirror.backendRef.port !== undefined)
                          bFilterObj.requestMirror.backendRef.port =
                            bFilter.requestMirror.backendRef.port;
                        bFilterObj.requestMirror.backendRef.namespace =
                          namespace || "default";
                      }
                      if (bFilter.requestMirror.percent !== undefined)
                        bFilterObj.requestMirror.percent =
                          bFilter.requestMirror.percent;
                      if (bFilter.requestMirror.fraction)
                        bFilterObj.requestMirror.fraction =
    ...
    if (configMapSpec.immutable !== undefined)
      spec.immutable = configMapSpec.immutable;
    ...
    if (
      configMapSpec.binaryData &&
      Object.keys(configMapSpec.binaryData).length > 0
    ) {
    ...
    if (secretSpec.immutable !== undefined)
      spec.immutable = secretSpec.immutable;

    if (
      secretSpec.stringData &&
      Object.keys(secretSpec.stringData).length > 0
    ) {
      spec.stringData = addTrailingNewlinesToMultilineValues(
        secretSpec.stringData,
      );
    }
    ...
    if (limitRangeSpec.limits && Array.isArray(limitRangeSpec.limits)) {
      spec.limits = limitRangeSpec.limits
        .map((item: any) => {
          const cleanedItem: Record<string, any> = {};

          if (item.type) cleanedItem.type = item.type;
          if (item.default && Object.keys(item.default).length > 0)
            cleanedItem.default = item.default;
          if (
            item.defaultRequest &&
            Object.keys(item.defaultRequest).length > 0
          )
            cleanedItem.defaultRequest = item.defaultRequest;
          if (item.max && Object.keys(item.max).length > 0)
            cleanedItem.max = item.max;
          if (item.min && Object.keys(item.min).length > 0)
            cleanedItem.min = item.min;
          if (
            item.maxLimitRequestRatio &&
            Object.keys(item.maxLimitRequestRatio).length > 0
          )
            cleanedItem.maxLimitRequestRatio = item.maxLimitRequestRatio;
    ...
    if (runtimeClassSpec.handler !== undefined)
      spec.handler = runtimeClassSpec.handler;

    if (
      runtimeClassSpec.overhead &&
      Object.keys(runtimeClassSpec.overhead).length > 0
    ) {
      const overhead: Record<string, any> = {};
      if (
        runtimeClassSpec.overhead.podFixed &&
        Object.keys(runtimeClassSpec.overhead.podFixed).length > 0
      ) {
    ...
    if (runtimeClassSpec.scheduling) {
      ...
      if (
        runtimeClassSpec.scheduling.tolerations &&
        Array.isArray(runtimeClassSpec.scheduling.tolerations) &&
        runtimeClassSpec.scheduling.tolerations.length > 0
      ) {
        scheduling.tolerations = runtimeClassSpec.scheduling.tolerations
          .map((tol: any) => {
            ...
            if (tol.tolerationSeconds !== undefined)
              cleanedTol.tolerationSeconds = tol.tolerationSeconds;
            return cleanedTol;
          })
